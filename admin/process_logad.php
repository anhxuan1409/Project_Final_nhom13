<?php
    ...
    if(isset($_POST['btnSignIn'])){
        ...
        $sql = "SELECT * FROM db_admin WHERE admin_nickname = '$email' AND admin_pass='$pass'";
        ...
        if(mysqli_num_rows($result) > 0){
            // CẤP THẺ LÀM VIỆC
            $_SESSION['ADLoginOK'] = $email;
            header("location: admin.php"); //Chuyển hướng về Trang quản trị
        }else{
            $error = "Bạn nhập thông tin Email hoặc mật khẩu chưa chính xác";
            header("location: logad.php?error=$error"); //Chuyển hướng, hiển thị thông báo lỗi
        }
